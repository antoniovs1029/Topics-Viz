...
def distributions(ts_id):
    tset = db.session.query(TopicSet).filter(TopicSet.id == ts_id).one() # para que si no existe el topicset, suceda un error
    distribs = TopicWordDistribution.query.filter_by(topicset_id = tset.id).all()
    return render_template('distributions.html', title = "Distribuciones", ts_id = tset.id, distribs = distribs)
